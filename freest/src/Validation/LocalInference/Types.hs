...
match e modl = match' e modl Set.empty Set.empty
  ...
  match'' e modl bindings visited t1 t2 = do
    ...
    case (t1, t2) of
      ...
      (T.AppSemi _ (T.AppMessage _ m1 p1 t11) t12, T.AppMessage s m2 p2 t21)
        | m1 == m2 && p1 == p2
        -> do
        mcsθ1 <- match' e modl bindings visited t11 t21
        mcsθ2 <- case m1 of
      ...
      (T.AppDual _ (T.AppVar _ a1 k1 vm1 t1s), T.AppDual _ (T.AppVar _ a2 k2 vm2 t2s))
        ...
        -> do
        mcsθ <- match' e modl bindings visited (T.Var (getSpan a1) k1 vm1 a1)
                                               (T.Var (getSpan a2) k2 vm2 a2)
        foldMatch e modl bindings visited mcsθ t1s t2s
      -- M-Const (M-ArrowL/R)
      (T.App _ t1' t1s, T.App _ t2' t2s)
        ...
        -> foldMatch e modl bindings visited (mcs, emptySubs) t1s t2s
      ...
      (T.AppQuant s1 p1 pk1 m1 ((a1, k1) : aks1) t1', T.AppQuant s2 p2 pk2 m2 ((a2, k2) : aks2) t2')
        | p1 == p2 && pk1 == pk2
        -> first (mcs ++) <$> match' e modl (Set.insert (a1, a2) bindings) visited
      ...
      -- M-RedexL
        ...
        -> let visited' = Set.insert (fromJust $ tNameRedex t1) visited
            in match' e modl bindings visited' (normalise modl t1) t2
      -- M-RedexR
        | isJust (tNameRedex t2)
        -> let visited' = Set.insert (fromJust $ tNameRedex t2) visited 
            in match' e modl bindings visited' t1 (normalise modl t2)
  ...
  foldMatch e modl bindings visited (mcs, θ) t1s t2s =
    (<> (mcs, θ)) . fold <$> zipWithM (match' e modl bindings visited) t1s t2s
