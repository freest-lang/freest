...
type TypeDecls p = ModuleAssoc p Identifier (Int, T.Type p)
...
insertTypeDecl :: Identifier
               ...
               -> ParsedModule
               -> ParsedModule
insertTypeDecl i aks t m = m{typeDecls = (i, (length aks, t')) : typeDecls m}
...
instance Show ParsedModule where
  show Module{name,imports,kindSigs,dataDecls,typeDecls,definitions} =
    ...
    where 
      ...
      showTypeDecl (i, (n, T.Abs _ _ aks t)) | n > 0 = 
        "type " ++ show i ++ " " ++ unwords (map show aks) ++ " = " ++ show t
      showTypeDecl (i, (_, t)) = "type " ++ show i ++ " = " ++ show t
