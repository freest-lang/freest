{- |
Module      :  Exchange a binary tree on a channel, an HO version
Description :  The producer, rather than sending the integer directly on the
channel, introduces an indirection: sends a channel on with the integer shall be
sent.
Copyright   :  (c) LASIGE and University of Lisbon, Portugal
Maintainer  :  balmeida@lasige.di.fc.ul.pt
-}

-- The channel type, as seen from the producer side

type TreeChannel : 1C
type TreeChannel = TreeC; Close

type TreeC : 1S
type TreeC = +{
  LeafC: Skip,
  NodeC: TreeC; !(?Int; Close); TreeC
 }

-- Reading a channel end: consuming Dual TreeChannel

receiveCh : forall a -> ?(?Int; Close); a -> (Int, a)
receiveCh @a c =
  let (r, c) = receive c in
  let x = receiveAndClose r in
  (x, c)

read : forall a -> Dual TreeC; a -> (Tree, a)
read @a (&LeafC c) = (Leaf, c)
read @a (&NodeC c) =
  let (l, c) = read c in
  let (x, c) = receiveCh c in
  let (r, c) = read c in
  (Node l x r, c)

readTree : Dual TreeChannel -> Tree
readTree r = 
  let (tree, r) = read r in 
  wait r;
  tree

-- Writing a tree on a channel: consuming TreeChannel
type Tree : *T
data Tree = Leaf | Node Tree Int Tree

sendCh : forall a -> Int -> !(?Int; Close); a -> a
sendCh @a x c =
  let (r, w) = channel @(?Int; Close) in
  let c = send r c in
  sendAndWait x w;
  c

write : forall a -> Tree -> TreeC ; a -> a
write @a Leaf c = select LeafC c
write @a (Node l x r) c = 
  c |> select NodeC
    |> write l
    |> sendCh x
    |> write r

writeTree : Tree -> TreeChannel -> ()
writeTree tree writer = writer |> write tree |> close

-- Go: transmit aTree

aTree : Tree
aTree = Node (Node Leaf 5 Leaf) 7 (Node (Node Leaf 11 Leaf) 9 (Node Leaf 15 Leaf))

main : ()
main = forkWith (writeTree aTree) |> readTree |> print
