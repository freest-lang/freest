...
main =
  ...
  -- fork #1 (\(_ : ()) -1-> sendTree aTree w);   -- No error
  fork #1 (\(_ : ()) -1-> writeNothing w);             -- Error: Premature EndOfStream
  -- fork #1 (\(_ : ()) -1-> writeTooMuch w);      -- Error: Extraneous elements in the stream after reading a full tree
  -- fork #1 (\(_ : ()) -1-> writeRootTreeOnly w); -- "Error: Empty stack on right subtree"
  -- fork #1 (\(_ : ()) -1-> writeLeftTreeOnly w); -- "Error: Empty stack on left subtree",
  receiveTree r
