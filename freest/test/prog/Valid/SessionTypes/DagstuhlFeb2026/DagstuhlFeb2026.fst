...
length @a (&More (?_ ; c)) = 1 + length @a c 
...
showStream c =
  let (x, c) = c |> sendType @String |> send ""
                 ...
                 |> send (\(x:String) (y:Int) -> (++) @Char x (showInt y)) |> send 5
                 |> select More |> sendType @Bool
                 |> send (\(x:String) (y:Bool) -> (++) @Char x (showBool y)) |> send True
                 |> select Done
                 |> receive
  in close c ; x
...
main : ()
main =
  forkWith @(Dual Fold) @() fold |>
  showStream |>
  print @String -- expect "5True"
