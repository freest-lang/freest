...
bagServer state serverChannel =
  ...
  fork #1 (\(_ : ()) -1-> handleClient state serverSide);
  bagServer state serverChannel
...
main =
  ...
  fork #1 (\(_ : ()) -1-> emptyBagServer serverSide);
  fork #1 (\(_ : ()) -1-> put 7 clientSide);
  fork #1 (\(_ : ()) -1-> put 5 clientSide);
  fork #1 (\(_ : ()) -1-> put 1 clientSide);
